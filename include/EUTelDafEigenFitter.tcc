...
namespace daffitter{
  ...
  template <typename T, size_t N>
  void EigenFitter<T,N>::updateInfo(const FitPlane<T> &pl, const int index, TrackEstimate<T,N> *e){
...
template <typename T, size_t N>
void EigenFitter<T,N>::getKFGain(const FitPlane<T>  &p1, TrackEstimate<T,N>* e){
...
template <typename T, size_t N>
void EigenFitter<T,N>::kfUpdate(const FitPlane<T>  &pl, int index, TrackEstimate<T,N>* e){
...
template <typename T, size_t N>
void EigenFitter<T,N>::getAvg(TrackEstimate<T,N>* e1, TrackEstimate<T,N>* e2, TrackEstimate<T,N>* result){
  //Calculate the weighted average of two estimates
  if(e1->isSeed()){ result->copy(e2); return;}
  if(e2->isSeed()){ result->copy(e1); return;}
